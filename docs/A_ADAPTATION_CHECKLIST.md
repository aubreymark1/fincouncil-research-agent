...
| A-005 | 基本面、新闻政策、风险分析节点 | 基础实现已完成并合并；CONTRACT-CHANGE-002 补齐 multiple 来源输入 | PR #7 |
| A-006 | Critic | 已完成并合并 | PR #11 |
| A-007 | 报告生成 | 未完成 | — |
| A-008 | 第一次 B/C 集成 | 未完成 | — |

任务看板已按 A 角色手册统一编号：A-003 为 ModelProvider，A-004 为最小编排，后续以任务看板为唯一状态源。
...
|---|---|---|---|---|---|
| ADAPT-001 | 调用 B 的 `load_manifest` 后立即调用 `validate_manifest`，替换 `_stub_load_manifest` | B-001 | `app/main.py`、`app/orchestrator/graph.py` | 将返回的 `ValidationIssue` 写入 `ResearchState.validation_issues`；`error`/`critical` 阻止资料进入时间锁和后续分析，`warning`/`info` 可继续但不得丢弃；`load_manifest` 的 E100/E101/E102 异常直接使本次运行失败 | B-001 已完成，待 A 接入 |
| ADAPT-002 | 用 B 的 PDF/HTML 提取函数替换 fixture extractor | B-002、B-003 | `app/orchestrator/graph.py` | `TextChunk` 保留 `doc_id`、页码和定位 | B-002 已完成，待 B-003 |
| ADAPT-003 | 用 B 的 `locate_evidence` 替换 `_load_fixture_evidence` | B-004、B-005 | `app/orchestrator/graph.py` | 显式传入 documents 与 evidence_type；Evidence 元数据来自 SourceDocument，自动匹配保持 pending | CONTRACT-CHANGE-002 已确认，待 B 对齐 |
...
| ADAPT-011 | 为 multiple 指标提供上游独立来源确认 | B-005、manifest 去重/人工核验 | B/C 上游结果、`app/orchestrator/state.py` | documents 显式传入；至少两个不同 publisher 且 content_hash 不同；否则 unresolved | CONTRACT-CHANGE-002 已确认，A 基础实现已补齐 |

## 五、A 仍需并行完成的核心任务

这些任务不必等待 B/C，可以用公共 Schema 和共享 fixture 先完成：

1. **A-005 分析节点**：基础实现已在 PR #7 合并；风险证据逐类型相关性已补测，multiple 指标已按 CONTRACT-CHANGE-002 接收 documents；缺少独立发布主体时仍保持 unresolved。
2. **A-006 Critic**：已在 PR #11 合并；覆盖截止日期、证据状态、数字来源、定位、必查指标、冲突证据与稳定问题 ID。
